...
<template>
  <div class="full-list-pokemon-page">
    <!-- Componente Search -->
    <SearchComponent @data-pokemon-search="searchPokemonEvent" @pokemon-not-found="showNotFound = true" />

    <!-- Renderizado condicional basado en `showNotFound` -->
    <template v-if="!showNotFound">
      <ListComponent :dataList="dataSend" @data-pokemon-search="searchPokemonEvent"
    ...
    <ModalDetails v-if="dataModal" :dataPokemon="dataModal" @close-modal="closeModal"
      @data-pokemon-change-favorite="changePokemonFavorite" />
  </div>
</template>
